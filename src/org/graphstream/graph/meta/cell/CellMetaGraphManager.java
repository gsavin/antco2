...
public class CellMetaGraphManager
	extends OGANMetaGraphManager
{
		...
		extends HashMap<String,Cell>
	{
		...
		public Cell newCell()
		{
			Cell c = (Cell) metaGraph.addNode( String.format("cell-%s-%d",prefix,currentCellId) );
			put(c.getId(),c);
			return c;
		}
	}
	...
	protected Cell fusion( LinkedList<String> toFusion )
	{
		...
		for( Cell cell : cellsToMerge )
		{
			...
			for( String edgeId : cell.eachEdgeId() )
			{
				...
				lamama.addEdge(edgeId,edge.getSourceNode().getId(),
			...
			for( Edge preEdge : cell )
			{
				...
				if( oldMetaEdge.getOpposite(cell) == lamama )
				{
					for( String edgeId : oldMetaEdge.eachEdgeId() )
					{
						...
						lamama.addEdge(edge.getId(),edge.getSourceNode().getId(),
								edge.getTargetNode().getId(),edge.isDirected());
					}
				}
				else
				{
					MetaEdge metaEdge = (MetaEdge) lamama.getEdgeToward(opposite.getId());
					
					if( metaEdge == null )
						metaEdge = (MetaEdge) metaGraph.addEdge( String.format("metaedge-%s-%s",lamama.getId(),opposite.getId()),
								lamama.getId(), opposite.getId(), false );
	...
	public void mitose( Cell cell, LinkedList<String> nodeToRetain )
	{
		...
		for( String edgeId : cell.eachEdgeId() )
		{
			...
			if( nodeToRetain.contains(edge.getSourceNode().getId()) !=
				nodeToRetain.contains(edge.getTargetNode().getId()) )
			{
				MetaEdge metaEdge = (MetaEdge) cell.getEdgeToward(newCell.getId());
				
				if( metaEdge == null )
					metaEdge = (MetaEdge) metaGraph.addEdge(
							String.format("metaedge-%s-%s", cell.getId(), newCell.getId() ),
							cell.getId(),
							newCell.getId(),
							false);
			...
			{
				newCell.addEdge(edgeId, edge.getSourceNode().getId(),
						edge.getTargetNode().getId(), edge.isDirected() );
				cell.removeEdge(edgeId);
			}
		}
