...
public class ONOGMetaGraphManager
	extends SinkAdapter implements MetaGraphManager
{
	...
	class MetaEdgeFactory
		implements EdgeFactory
	{
		public Edge newInstance(String id, Node src, Node dst, boolean directed)
		{
			return new ONOGMetaEdge(id,(ONOGMetaNode)src,(ONOGMetaNode)dst,directed);
		}
	}
	...
	public void nodeRemoved( String sourceId, long timeId, String nodeId )
	{
		...
		{
			ONOGMetaNode metaNode = (ONOGMetaNode) metaGraph.getNode(metaIndexes.get(nodeId));
			...
			String nodeId, String attributeId, Object olValue, Object newValue )
	{
		...
		{
			if( metaIndexes.containsKey(nodeId) && ! metaIndexes.get(nodeId).equals(metaIndexUndefined) )
				metaGraph.getNode(metaIndexes.get(nodeId)).setAttribute("ui.color",newValue);
		}
	...
	{
		if( MetaGraph.debug )
			System.out.printf("[meta] metagraph, metaindex of \"%s\" changed from \"%s\" to \"%s\"%n",nodeId,metaIndexes.get(nodeId),metaIndex);
		...
		if( metaGraph.getNode(metaIndex) == null )
		{
			...
			if( metaIndex.equals(metaIndexUndefined) )
				metaGraph.getNode(metaIndexUndefined).addAttribute("ui.class","undefined");
	...
	{
		ONOGMetaNode current = (ONOGMetaNode) metaGraph.getNode(metaIndexes.get(nodeId));
		...
		{
			if( MetaGraph.debug )
				System.out.printf("[meta] metagraph, delete metanode \"%s\"%n",current.getId());
		...
		while( edgesToRecreate.size() > 0 )
		{
			...
			addInnerEdge( ed.edgeId, ed.sourceNodeId, 
					ed.targetNodeId, ed.directed);
		}
	}
	...
	{
		if( MetaGraph.debug )
			System.out.printf("[meta] metagraph, edgeAdded \"%s\" from \"%s\" to \"%s\"%n",edgeId,sourceNodeId,targetNodeId);
		
		addInnerEdge(edgeId,sourceNodeId,targetNodeId,directed);
	}
	
	protected void addInnerEdge( String edgeId,
			String sourceNodeId, String targetNodeId, boolean directed )
	{
		if( MetaGraph.debug )
			System.out.printf("[meta] metagraph, addEdge \"%s\" from \"%s\" to \"%s\"%n\tfrom meta \"%s\" to meta \"%s\"%n",edgeId,sourceNodeId,targetNodeId,metaIndexes.get(sourceNodeId),metaIndexes.get(targetNodeId));
		
		ONOGMetaNode sourceMetaNode = (ONOGMetaNode) metaGraph.getNode(metaIndexes.get(sourceNodeId));
		ONOGMetaNode targetMetaNode = (ONOGMetaNode) metaGraph.getNode(metaIndexes.get(targetNodeId));
		
		edgeSourceIndexes.put(edgeId,sourceMetaNode.getId());
		
		if( sourceMetaNode == targetMetaNode )
		{
			sourceMetaNode.addEdge(edgeId,sourceNodeId,targetNodeId,directed);
		}
		else
		{
			if( sourceMetaNode.getEdgeToward(targetMetaNode.getId()) == null )
			{
				...
				metaGraph.addEdge(metaEdgeId,sourceMetaNode.getId(),targetMetaNode.getId(),false);
				
				if( MetaGraph.debug )
					System.out.printf("[meta] metagraph, add metaedge \"%s\"%n",metaEdgeId);
			}
			
			sourceMetaNode.addEdge(edgeId,sourceNodeId,targetNodeId,directed);
			targetMetaNode.addEdge(edgeId,sourceNodeId,targetNodeId,directed);
		}
	}
	...
	public void removeInnerEdge( String edgeId )
	{
		...
		{
			ONOGMetaNode metaNode = (ONOGMetaNode) metaGraph.getNode(edgeSourceIndexes.get(edgeId));
			
			String op = metaNode.getInnerGraph().getEdge(edgeId).getTargetNode().getId();
			...
			{
				ONOGMetaEdge metaEdge = (ONOGMetaEdge) metaNode.getEdgeToward(metaIndexes.get(op));
				if( metaEdge.size() == 0 )
				{
					if( MetaGraph.debug )
						System.out.printf("[meta] metagraph, remove metaedge \"%s\"%n",metaEdge.getId());
