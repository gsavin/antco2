...
public class ONOGMetaNode extends MultiNode {
	private static class __element_to_id_iterator implements Iterator<String> {
	...
	private class __iterable_nodes implements Iterable<String> {
	...
	private class __iterable_edges implements Iterable<String> {
		@SuppressWarnings("all")
		public Iterator<String> iterator() {
			return new __element_to_id_iterator(
					(Iterator<? extends Element>) innerGraph.getEdgeIterator());
		}
	}
	...
	public Node addNode(String nodeId) {
		if (MetaGraph.debug)
			System.out.printf("[meta] metanode \"%s\", addnode \"%s\"%n",
					getId(), nodeId);

		setAttribute("ui.style", "size: " + (innerGraph.getNodeCount() + 5)
				+ "px;");
		...
		if (innerGraph.getNode(nodeId) == null)
			...
			if (isVirtual(n)) {
				...
				/*
				 * for( Edge e : n ) { if( ! isVirtual( e.getOpposite(n) ) )
				 * e.removeAttribute("meta.virtual"); }
				 */
			} else {
				System.err.printf(
						"something is strange ... trying to add node \"%s\"\n",
						nodeId);
			}
	...
	public void removeNode(String nodeId) {
		if (MetaGraph.debug)
			System.out.printf("[meta] metanode \"%s\", remove node \"%s\"%n",
					getId(), nodeId);
	...
	public Edge addEdge(String edgeId, String srcNodeId, String trgNodeId,
			boolean directed) {
		if (MetaGraph.debug)
			System.out.printf("[meta] metanode \"%s\", add edge \"%s\"%n",
					getId(), edgeId);
		...
		boolean virtualEdge = isVirtualNode(srcNodeId)
				|| isVirtualNode(trgNodeId);
	...
	public void removeEdge(String edgeId) {
		if (MetaGraph.debug)
			System.out.printf("[meta] metanode \"%s\", remove edge \"%s\"%n",
					getId(), edgeId);
