...
public class ONOGMetaEdge extends MultiEdge {
	class VirtualEdgeSink extends SinkAdapter {
		...
		protected void addEdge(String sourceId, String edgeId) {
			...
			if (MetaGraph.debug)
				System.out
						.printf("[meta] metaedge \"%s\", add edge \"%s\" from \"%s\"%n",
								getId(), edgeId, sourceId);
			...
			if (target.getInnerGraph().getNode(virtualNodeId) != null
				...
				System.err
						.printf("something strange... should not be virtual\n");
		}

		public void edgeRemoved(String sourceId, long timeId, String edgeId) {
			if (MetaGraph.debug)
				System.out.printf(
						"[meta] metaedge \"%s\", remove edge \"%s\"%n",
						getId(), edgeId);
