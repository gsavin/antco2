...
public class OGANMetaGraphManager
	extends SinkAdapter implements MetaGraphManager
{
		...
		implements EdgeFactory
	{
		...
		{
			return new OGANMetaEdge(graph,id,(OGANMetaNode)src,(OGANMetaNode)dst,directed);
		}
	}
	...
	public void init(MetaGraph metaGraph, Source source)
	{
		...
		{
			graph = new MultiGraph( String.format( "%s-shared-graph", metaGraph.getId() ) );
		}
			...
			String nodeId, String attributeId, Object olValue, Object newValue )
	{
		...
		{
			String metaNodeIndex = (String) graph.getNode(nodeId).getAttribute("meta.node");
			
			if( metaNodeIndex != null && ! metaNodeIndex.equals(metaIndexUndefined) )
				metaGraph.getNode(metaNodeIndex).setAttribute("ui.color",newValue);
		}
	...
	protected void nodeMetaIndexChanged( String nodeId, String metaIndex )
	{
		...
		MetaNode currentMetaNode = (MetaNode) metaGraph.getNode(currentMetaNodeId);
		...
		if( MetaGraph.debug )
			System.out.printf("[meta] manager, set meta.node of \"%s\" to \"%s\"%n", nodeId, metaIndex );
	...
	public void nodeRemoved( String sourceId, long timeId, String nodeId )
	{
		...
		MetaNode metaNode = (MetaNode) metaGraph.getNode( (String) n.getAttribute("meta.node") );
		...
		for( int i = 0; i < n.getDegree(); i++ )
		{
			...
			{
				MetaEdge metaEdge = (MetaEdge) metaGraph.getEdge( (String) e.getAttribute("meta.edge") );
	...
	protected void addInnerEdge( Edge e )
	{
		...
		String sourceMetaNodeId = (String) e.getSourceNode().getAttribute("meta.node");
		String targetMetaNodeId = (String) e.getTargetNode().getAttribute("meta.node");
		
		if( sourceMetaNodeId.equals(targetMetaNodeId) )
		{
			MetaNode metaNode = (MetaNode) metaGraph.getNode( sourceMetaNodeId );
			metaNode.addEdge(e.getId(),e.getSourceNode().getId(),e.getTargetNode().getId(),e.isDirected());
		}
		else
		{
			MetaNode metaNode = (MetaNode) metaGraph.getNode( sourceMetaNodeId );
			MetaEdge metaEdge = (MetaEdge) metaNode.getEdgeToward(targetMetaNodeId);
			
			if( metaEdge == null )
			{
				metaEdge = (MetaEdge) metaGraph.addEdge( String.format("metaedge-%s-%s",sourceMetaNodeId,targetMetaNodeId),
						sourceMetaNodeId, targetMetaNodeId, false );
				
				if( MetaGraph.debug )
					System.out.printf("[meta] manager, add metaedge \"%s\" from \"%s\" to \"%s\"%n",
							metaEdge.getId(), sourceMetaNodeId, targetMetaNodeId );
			}
			
			metaEdge.addEdge(e.getId());
			
			if( MetaGraph.debug )
				System.out.printf("[meta] manager, set meta.edge of \"%s\" to \"%s\"%n", e.getId(), metaEdge.getId() );
			
			graph.getEdge(e.getId()).setAttribute("meta.edge",metaEdge.getId());
		}
	}
	...
	protected void removeInnerEdge( Edge e )
	{
		...
		if( ! e.hasAttribute("meta.edge") )
		{
			MetaNode metaNode = (MetaNode) metaGraph.getNode( (String) e.getSourceNode().getAttribute("meta.node") );
			metaNode.removeEdge(e.getId());
		}
		else
		{
			...
			if( MetaGraph.debug )
				System.out.printf("[meta] meta edge of \"%s\" is \"%s\"%n", e.getId(), (String) e.getAttribute("meta.edge") );
			...
			{
				if( MetaGraph.debug )
					System.out.printf("[meta] manager, remove metaedge \"%s\"%n", metaEdge.getId() );
