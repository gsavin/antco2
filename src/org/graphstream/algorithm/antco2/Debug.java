...
public class Debug implements Sink {
	static FileWriter out;
	static String path = "debug-trace-2";

	public static void log(String msg) {
		/*
		 * if( out == null ) try { out = new FileWriter(path);
		 * Runtime.getRuntime().addShutdownHook( new Thread() { public void
		 * run() { try { out.close(); } catch (IOException e) {
		 * e.printStackTrace(); } } }); } catch( IOException e ) {
		 * e.printStackTrace(); }
		 * 
		 * try { out.write(msg); out.flush(); } catch( IOException e ) {
		 * e.printStackTrace(); }
		 */
	}
