...
public class ReadBoids
{
	...
	{
		readAndAddCoord(args[0],args[1]);
		//justRead();
		//writePictures();
		//countEvents();
	}
	
	protected static class InnerRenderer
		extends SwingBasicGraphRenderer
	{
	...
	public static void writePictures()
	{
		...
		g.addAttribute( "ui.stylesheet", "graph { padding: 50px; fill-color: black; } node { fill-mode: dyn-plain; fill-color: #5782db,#90dd3e,#e069cb,#e0ce69,#e07c69; } edge { fill-color: white; }" );
		...
		try {
			...
			while( g.getEdgeCount() == 0 ) dgs.nextStep();
		...
		try {
			...
			BufferedImage current = new BufferedImage(1920,1080,BufferedImage.TYPE_INT_ARGB);
			...
			while( live )
			{
				...
				current.flush();
				ImageIO.write(current,"PNG",new File(String.format( "%s%05d.png", screenshotPrefix, screenshotCount++)));
				
				System.out.printf("\033[s%d images written (%d ms/image)\033[u",screenshotCount-1,System.currentTimeMillis()-m1);
	...
	public static void justRead()
	{
		...
		try {
			...
			while( live )
			{
				g.addAttribute( "ui.screenshot", String.format( "%s%05d.png", screenshotPrefix, screenshotCount++) );
	...
	public static void readAndAddCoord( String in, String out )
	{
		FileSourceDGS dgs = new FileSourceDGS();
		...
		try {
			while( dgs.nextEvents() )
			{
				...
				{
					sbox.compute();
					System.out.printf("\033[s\033[K%d/%d (%d%%)\033[u", size-fin.available(),size,100*(size-fin.available())/size);
