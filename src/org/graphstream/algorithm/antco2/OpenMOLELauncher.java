...
public class OpenMOLELauncher
{
	public static class Args
	{
		...
		public void parse( String ... args )
		{
			...
			for( int i = 0; i < args.length; i++ )
			{
				...
				else if( args [i].matches( "^--(graph-size|iteration|colonies)=\\d+$" ) )
				{
					int k = Integer.parseInt( args[i].substring( args[i].indexOf('=')+1 ) );
	...
	public static void main(String[] toParse)
	{
		...
		System.out.printf( "Usage: java %s options%n",OpenMOLELauncher.class.getName());
		System.out.printf( "where options is : %n\t--(enable|disable)-(display|randomize)%n\t--(graph-size|iteration|colonies)=integer%n");
		System.out.printf( "display:    %s%nrandomize:  %s%ngraph size: %d%niterations: %d%ncolonies:   %d%n=========%n",
				args.display, args.randomize, args.size, args.iteration, args.colonies );
		...
		for( int i = 0; i < args.colonies; i++ )
			g.addAttribute( "antco2.resources", String.format( "+ colonies-%03x", i ) );
